...
public class PreviewViewController: UIViewController {
    ...
    lazy var rightBarButton = UIButton(frame: CGRect(x: 0, y: 0, width: 44, height: 44))
    ...
    private var currentAsset: PHAsset! {
        didSet {
            ...
            let title = isSelected ? "\(currentAsset.selectionOrder + 1)" : nil
            let bundle = Bundle(for: AssetsViewController.self)
            let image = isSelected ? nil : UIImage(named: "pic_unselect", in: bundle, compatibleWith: nil)
            presentationRightbutton?.backgroundColor = isSelected ? .red : .clear
            presentationRightbutton?.setTitle(title, for: .normal)
            presentationRightbutton?.setImage(image, for: .normal)
        }
    }
...
//        rightBarButton.backgroundColor = .yellow
        let presentationRightbutton = UIButton(frame: CGRect(x: 0, y: 0, width: 20, height: 20))
        self.presentationRightbutton = presentationRightbutton
        rightBarButton.addSubview(presentationRightbutton)
        presentationRightbutton.translatesAutoresizingMaskIntoConstraints = false
        presentationRightbutton.centerYAnchor.constraint(equalTo: rightBarButton.centerYAnchor).isActive = true
        presentationRightbutton.rightAnchor.constraint(equalTo: rightBarButton.rightAnchor).isActive = true
        presentationRightbutton.widthAnchor.constraint(equalToConstant: 20).isActive = true
        presentationRightbutton.heightAnchor.constraint(equalToConstant: 20).isActive = true
        presentationRightbutton.isUserInteractionEnabled = false
        presentationRightbutton.makeCorners(cornerRadius: 10)
        presentationRightbutton.contentHorizontalAlignment = .center
        presentationRightbutton.titleLabel?.textAlignment = .center
        presentationRightbutton.titleLabel?.font = UIFont.systemFont(ofSize: 12)
        rightBarButton.setNeedsLayout()
    ...
    func setupCollectionView() {
        ...
        let bundle = Bundle(for: AssetsViewController.self)
        collectionView.register(UINib(nibName: "PriviewCollectionViewCell", bundle: bundle), forCellWithReuseIdentifier: kPreviewCellID)
